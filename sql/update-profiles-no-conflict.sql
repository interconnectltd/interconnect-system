...
-- ON CONFLICTを削除（emailにユニーク制約がないため）
...
BEGIN
    -- 田中太郎が存在しない場合のみ挿入
    IF NOT EXISTS (SELECT 1 FROM profiles WHERE email = 'tanaka@example.com' OR name = '田中太郎') THEN
    ...
    -- 佐藤花子が存在しない場合のみ挿入
    IF NOT EXISTS (SELECT 1 FROM profiles WHERE email = 'sato@example.com' OR name = '佐藤花子') THEN
        INSERT INTO profiles (
            id,
            name,
            email,
    ...
    -- 山田美咲が存在しない場合のみ挿入
    IF NOT EXISTS (SELECT 1 FROM profiles WHERE email = 'yamada@example.com' OR name = '山田美咲') THEN
        INSERT INTO profiles (
            id,
            name,
            email,
