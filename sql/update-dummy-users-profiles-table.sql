-- profilesテーブルを使用したダミーユーザーのデータ多様化SQL
-- user_profilesではなくprofilesテーブルにtitleカラムが存在

-- まず既存のダミーユーザーがいるか確認
-- SELECT * FROM profiles WHERE display_name IN ('りゅう', 'guest');
...
-- FROM profiles
-- WHERE display_name IN ('りゅう', 'guest', '田中太郎', '佐藤花子', '山田美咲')
