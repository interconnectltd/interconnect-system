-- =====================================================
-- ドメイン移行: 招待リンク生成関数のURL更新
-- 実行場所: Supabase SQL Editor
-- 目的: 関数内の旧URL → 新URL に更新
-- =====================================================

-- 1. create_invite_link 関数を再作成（最新版 = fix-create-invite-link-final.sql 相当）
...
BEGIN
    ...
    BEGIN
        INSERT INTO invite_links (
            created_by, link_code, description, is_active, max_uses, used_count
        ) VALUES (
            p_user_id, v_link_code, p_description, true, NULL, 0
        )
        ...
        SELECT json_build_object(
            ...
            'full_url', 'https://inter-connect.app/register?ref=' || link_code,
...
BEGIN
    RETURN QUERY
    SELECT
        il.id, il.created_by, il.link_code, il.description,
