-- TimeRex予約システム用のテーブル作成（最終修正版）
-- 既存のデータベース構造に完全対応

-- 予約セッション管理テーブル
CREATE TABLE IF NOT EXISTS booking_sessions (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    timerex_session_id VARCHAR(255) UNIQUE NOT NULL, -- TimeRexのセッションID
...
CREATE TABLE IF NOT EXISTS bookings (
    ...
    timerex_session_id VARCHAR(255) REFERENCES booking_sessions(timerex_session_id),
...
CREATE OR REPLACE FUNCTION add_referral_points_booking(
    user_id UUID,
    points INTEGER,
    reason TEXT,
