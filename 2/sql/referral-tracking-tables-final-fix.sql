...
BEGIN
    ...
    ELSE
        ...
        CREATE TABLE invitations (
            ...
            invitation_code VARCHAR(20), -- 既存のカラム名を使用
        ...
        CREATE INDEX idx_invitations_invitation_code ON invitations(invitation_code);
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns 
               WHERE table_name='profiles' AND column_name='referral_points') THEN
        UPDATE profiles 
...
SELECT 
    ...
    COALESCE(booking_count.count, 0) as booking_count,
...
LEFT JOIN (
    -- bookingsテーブルにreferral_codeカラムがある場合のみカウント
    SELECT 
        referral_code, 
        COUNT(*) as count
    FROM (
        SELECT CASE 
            WHEN EXISTS (SELECT 1 FROM information_schema.columns 
                        WHERE table_name = 'bookings' AND column_name = 'referral_code') 
            THEN (SELECT COUNT(*) FROM bookings WHERE referral_code = il.link_code)
            ELSE 0 
        END as count,
        il.link_code as referral_code
        FROM invite_links il
    ) booking_subquery
    GROUP BY referral_code
) booking_count ON booking_count.referral_code = il.link_code
WHERE il.is_active = true
GROUP BY il.id, il.link_code, il.description, il.created_by, p.email, il.created_at, booking_count.count
